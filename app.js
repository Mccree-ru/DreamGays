...
const app = {
    ...
    async init() {
        ...
        try {
            ...
            document.getElementById('catalogGrid').innerText = "Ошибка загрузки профиля.";
            return;
        }
    ...
    async loadCatalog() {
        try {
            // Запрашиваем отфильтрованные и отсортированные данные напрямую из Supabase
            ...
            // Строим список авторов на основе полученных данных (только при первой загрузке без фильтров)
    ...
    renderCatalogGrid() {
        ...
        // Управление видимостью кнопки сброса
        const showReset = this.selectedGenreTab || this.selectedAuthor || this.sortPopularActive;
        document.getElementById('resetBtn').style.display = showReset ? "block" : "none";
    ...
    showScreen(screenId) {
        ...
        if(screenId === 'mainScreen') this.renderCatalogGrid();
    }
};
