...
const app = {
        ...
        window.supabase
            ...
            .on('postgres_changes', { 
                ...
                if (!this.userPurchasedIds.includes(newMangaId)) {
                    ...
                    // БАГФИКС: Точечное обновление замка (Не ломаем DOM)
    ...
    renderCatalogGrid(mangaArray, appendMode = false) {
        ...
        // БАГФИКС: Предотвращаем размножение обработчиков, убивающих свайпы
    ...
    // --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ТОЧЕЧНОГО ОБНОВЛЕНИЯ DOM ---
    ...
    async checkRecentPurchases(showNotification = false) {
        try {
            ...
            if (hasNew) {
                ...
                this.updateReadButtonUI(); 
                // Обновляем замок без перерисовки всей сетки
    ...
    async toggleLike() {
        ...
        // БАГФИКС 3: Снимаем фокус с кнопки Лайка, чтобы кнопки клавиатуры (Стрелочки/Пробел) 
        // случайно не нажимали ее повторно во время чтения
        if (document.activeElement) document.activeElement.blur();

        // ОПТИМИСТИЧНЫЙ UI: меняем данные сразу, не дожидаясь ответа базы данных
        ...
        // 1. Быстрое обновление интерфейса окна Превью
        this.updateLikeButtonUI();
        const finalLikes = this.currentManga.likes || this.currentManga.likes_count || 0;
        document.getElementById('previewLikes').textContent = `❤️ ${finalLikes}`;
        
        // 2. БАГФИКС 1 и 2: Точечное обновление бейджа на карточке в каталоге
        // Вместо renderCatalogGrid мы меняем только нужный HTML-блок, 
        // тем самым не плодя дубликаты событий контекстного меню.
        this.updateMangaCardStats(mangaId);

        try {
            // Тихо отправляем в базу
