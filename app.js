...
const app = {
	...
	async openMangaPreview(mangaId) {
        ...
        try {
            ...
            (allComments || []).forEach(comment => {
                // Если индекс страницы не указан (null/undefined), это главный коммент тайтла
                if (comment.page_index === null || comment.page_index === undefined) {
                    this.currentManga.cachedComments.push(comment);
                } else {
                    // Иначе распределяем коммент в кэш конкретной страницы
    ...
    // ИСПРАВЛЕНО: Метод больше не дергает базу данных при каждом вызове шторки. 
    // Он просто мгновенно отрисовывает то, что лежит в оперативной памяти (в кэше тайтла).
    loadMainComments() {
        const container = document.getElementById('mainCommentsScroll');
        if (!container) return;
    ...
    async sendMainComment() {
        ...
        try {
            ...
            // Отправляем комментарий в базу данных через API
            ...
            // ИСПРАВЛЕНО: Обновляем кэш в памяти локально, чтобы не скачивать всю ветку заново.
            // Если api.addComment возвращает объект созданного комментария (содержит id, created_at), пушим его.
            // Если API возвращает пустой ответ, собираем временный объект для мгновенного отображения.
            ...
            // Моментально обновляем счетчик на UI превью
            document.getElementById('previewComments').textContent = `💬 ${this.currentManga.comments_count}`;

            // Перерисовываем блок комментариев из обновленного кэша
            this.loadMainComments();
        } catch(e) {
    ...
    async deleteMainComment(commentId) {
        if(confirm("Удалить ваш комментарий к тайтлу?")) {
            ...
            try {
                ...
                // ИСПРАВЛЕНО: Локально удаляем коммент из кэша в памяти
                this.currentManga.cachedComments = this.currentManga.cachedComments.filter(c => String(c.id) !== String(commentId));

                // Обновляем счетчик на превью
                document.getElementById('previewComments').textContent = `💬 ${this.currentManga.comments_count}`;

                // Перерисовываем чат шторки без лишних запросов
                this.loadMainComments();
            } catch (e) {
