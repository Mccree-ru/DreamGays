const reader = {
    ...
    resetTimeout: null,
    isSendingComment: false, // Защита от спама

    // ИСПРАВЛЕНО: Убран массив preloadQueue для предотвращения утечек памяти OOM
    ...
    preloadSingleUrl(url, isPriority = false) {
        ...
        return new Promise((resolve) => {
            ...
            img.onload = () => {
                img.onload = null;
                img.onerror = null;
                img = null; // ИСПРАВЛЕНО: Мгновенное освобождение кучи
                resolve();
            };
            img.onerror = () => { 
    ...
    initTouchGestures() {
        ...
        track.addEventListener('touchmove', (e) => {
            ...
            if (e.touches.length === 1 && this.isDragging) {
                ...
                const img = container.querySelector('.reader-img');
                const viewWidth = img ? img.offsetWidth : window.innerWidth;
                const viewHeight = img ? img.offsetHeight : window.innerHeight;

                const maxMoveX = (viewWidth * (this.scale - 1)) / 2;
                const maxMoveY = (viewHeight * (this.scale - 1)) / 2;
...
// ТЕПЕРЬ СЕТЕВЫХ ЗАПРОСОВ НЕТ: Всё скачано заранее при открытии тайтла
    async loadCommentsForCurrentPage() {
        const container = document.getElementById('pageCommentsScroll');
        if (!container) return;

        // Безопасная проверка на существование объекта тайтла и карты кэша страниц
        if (!app.currentManga) return;
        if (!app.currentManga.cachedPagesComments) {
            app.currentManga.cachedPagesComments = {};
        }

        const pageKey = this.currentIndex;

        // Берем готовый массив из кэша (если там пусто — вернется пустой массив [])
        const pageComments = app.currentManga.cachedPagesComments[pageKey] || [];

        // Просто мгновенно отправляем массив на отрисовку в UI
        this.renderPageCommentsUI(pageComments);
    },
	
    // Вынесли отрисовку интерфейса в отдельный чистый метод, чтобы не дублировать код
    renderPageCommentsUI(commentsList) {
        const container = document.getElementById('pageCommentsScroll');
        if (!container) return;
    ...
    async sendPageComment() {
        ...
        try {
            ...
            const serverComment = await api.addComment(this.mangaId, this.currentIndex, app.userId, app.userName, text);
            input.value = "";

            // ИСПРАВЛЕНО: Добавляем новый комментарий сразу в локальный кэш текущей страницы в памяти
            const pageKey = this.currentIndex;
            if (app.currentManga && app.currentManga.cachedPagesComments) {
                if (!app.currentManga.cachedPagesComments[pageKey]) {
                    app.currentManga.cachedPagesComments[pageKey] = [];
                }
                
                if (serverComment && serverComment.id) {
                    app.currentManga.cachedPagesComments[pageKey].unshift(serverComment);
                } else {
                    app.currentManga.cachedPagesComments[pageKey].unshift({
            ...
            // Мгновенно перерисовываем интерфейс из обновленного локального кэша страницы
            this.renderPageCommentsUI(app.currentManga ? app.currentManga.cachedPagesComments[pageKey] : []);
        } catch (e) {
    ...
    async deletePageComment(commentId) {
        if (confirm("Удалить ваш комментарий к странице?")) {
            try {
                ...
                // Удаляем из бэкенда Supabase
                await api.deleteComment(commentId, app.userId);

                // ИСПРАВЛЕНО: Удаляем коммент из локального кэша этой страницы в оперативной памяти
                const pageKey = this.currentIndex;
                if (app.currentManga && app.currentManga.cachedPagesComments && app.currentManga.cachedPagesComments[pageKey]) {
                    app.currentManga.cachedPagesComments[pageKey] = app.currentManga.cachedPagesComments[pageKey].filter(c => String(c.id) !== String(commentId));
                }
                ...
                // Перерисовываем UI страницы без повторных обращений по URL
                this.renderPageCommentsUI(app.currentManga ? app.currentManga.cachedPagesComments[pageKey] : []);
            } catch (e) {
