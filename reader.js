const reader = {
    ...
    navigateTo(index, direction) {
        ...
        // НОВОВВЕДЕНИЕ: Выход при перелистывании последней страницы
        if (index >= this.pages.length) {
            if (window.Telegram?.WebApp?.HapticFeedback) {
                window.Telegram.WebApp.HapticFeedback.notificationOccurred('success');
            }
            if (window.app && typeof window.app.closeMangaReader === 'function') {
                window.app.closeMangaReader();
            }
            return;
        }
    ...
    applyZoom(scale, x = 0, y = 0) {
        ...
        const uiElements = document.querySelectorAll('.fab-comments, .reader-header');
    ...
    _onTouchEnd(e) {
        ...
        if (this._preventClick && !this._isPinching && this.scale === 1) {
            ...
            if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > minSwipeDistance) {
                ...
                // ИСПРАВЛЕНИЕ: Разрешаем свайпать "за пределы" массива, чтобы сработал выход
    ...
    initKeyboardAndMouseControls() {
        ...
        this._keydownHandler = (e) => {
            ...
            // ИСПРАВЛЕНИЕ: Убрали ограничение this.pages.length - 1 для автовыхода
            ...
            if (!isTrackpadSwipe) {
                ...
                if (this.scale > 1) {
                    ...
                    if (!this._isNavigating) {
                        ...
                        // ИСПРАВЛЕНИЕ: Убрали ограничение для выхода через трекпад
                        if (this._wheelAccumulatorX > 50) {
                            this.navigateTo(this.currentIndex + 1, 'forward');
    ...
    async loadCommentsForCurrentPage() {
        ...
        container.innerHTML = "<p style='color:var(--text-secondary); font-size:14px; text-align:center; margin-top:20px;'>Загрузка комментариев...</p>";
        
        try {
            const comments = await api.fetchPageComments(this.mangaId, this.currentIndex);
            if (!comments || comments.length === 0) {
                container.innerHTML = "<p style='color:var(--text-secondary); font-size:14px; text-align:center; margin-top:20px;'>Нет комментариев к этой странице.</p>";
                return;
            }
            ...
            comments.forEach(c => {
                ...
                const delBtnHtml = isMyComment ? `<span style="color:var(--danger-color); cursor:pointer;" onclick="reader.deletePageComment('${c.id}')">🗑</span>` : '';
            ...
            container.innerHTML = "<span style='color:var(--danger-color);'>Не удалось загрузить комментарии.</span>";
        }
    },
    ...
    // ОПТИМИЗАЦИЯ И ИСПРАВЛЕНИЕ: Абсолютно чистая выгрузка из памяти и сброс флагов
    destroy() {
        if (this._preloadTimer) clearTimeout(this._preloadTimer);
        if (this._navigationTimeout) clearTimeout(this._navigationTimeout);
        if (this._clickTimeout) clearTimeout(this._clickTimeout);
        ...
        // Отвязываем клавиатуру и затираем функцию
        if (this._keydownHandler) {
            window.removeEventListener('keydown', this._keydownHandler);
            this._keydownHandler = null; 
        }
        
        // БАГФИКС: Сбрасываем флаги инициализации! Иначе при повторном открытии тайтла
        // клавиатура и мышь не будут подключены.
        this.isGesturesInitialized = false;
        this.isPCControlsInitialized = false;
        ...
        const track = document.getElementById('readerTrack');
        if (track) {
            // Снимаем тяжелые слушатели тапов, чтобы они не копились в фоне
            track.removeEventListener('touchstart', this._handleTouchStart);
            track.removeEventListener('touchmove', this._handleTouchMove);
            track.removeEventListener('touchend', this._handleTouchEnd);
            track.removeEventListener('touchcancel', this._handleTouchEnd);
